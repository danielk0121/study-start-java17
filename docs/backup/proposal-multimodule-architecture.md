...
| Gradle 구성 | 멀티 프로젝트 |
| DB | 분리 없이 공유 |
| 실행 프로세스 | API 서버 4개 (member-service, order-service, auth-service, bff) |
...
향후 결제, 재고 차감 등이 추가되면 Saga 패턴(분산 트랜잭션 조율)과 Outbox 패턴(이벤트 발행 신뢰성 보장)을 함께 도입하는 것을 검토한다.
...
> 다른 서비스(order-service, auth-service, bff)도 동일한 패턴으로 작성한다.
> `COPY` 대상 모듈과 `EXPOSE` 포트만 서비스에 맞게 교체하면 된다.
