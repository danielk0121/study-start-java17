...
**주문 생성 시** (member 검증이 반드시 필요한 경우):
```
Circuit Breaker OPEN → fallback → 에러 응답 반환
"현재 서비스 점검 중입니다. 잠시 후 다시 시도해주세요."
```
검증을 생략할 수 없으니 주문 생성 자체를 막는다.

**주문 조회/취소 시** (member 정보가 보조 데이터인 경우):
```
Circuit Breaker OPEN → fallback → member 정보 없이 주문 데이터만 반환
{ orderId: 1, status: "PAID", member: null }
```
핵심 기능은 살리고 부가 데이터만 비운다.
...
**Q. 복잡한 fallback은 필요 없고, 임계치 설정에 따른 재시도나 포기 판단 기능만 있으면 충분할 것 같다.**

그게 실용적인 접근이다. fallback으로 partial response를 조합하는 건 Netflix 수준의 복잡한 서비스에서나 의미 있고, 대부분의 실무에서는 Circuit Breaker를 "일정 횟수 실패하면 빠르게 에러 반환하고, 복구되면 자동 재개" 용도로만 쓴다.
  ...
  circuitbreaker:
    instances:
      member-service:
        ...
        wait-duration-in-open-state: 10s # 10초 후 재시도
        sliding-window-size: 10
  timelimiter:
    instances:
      member-service:
        timeout-duration: 3s             # 3초 넘으면 포기
```

fallback은 에러 응답 하나로 통일하고, 복잡한 partial response는 짜지 않는다.
